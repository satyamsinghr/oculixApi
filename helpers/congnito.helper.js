...
class CognitoHelper {
  ...
  async changePassword(accessToken, oldPassword, newPassword) {
    try {
      ...
      console.log(paramsForChangePass);
  ...
  async requestConfirmPassword(emailId, otp, newPassword) {
    try {
      const params = this._getConfirmForgotPasswordParams(otp, emailId, newPassword);
      const passRes = await this.cognito.confirmForgotPassword(params).promise();
  ...
  async logout(accessToken) {
    try {
      ...
      console.log("ERROR: ", error);
      if (error.code === "NotAuthorizedException") return error.code;
      return false;
    }
  }
  ...
  _getCreateUserParams(emailId, resendCode = false) {
    ...
    if (resendCode) {
      createUserParams["MessageAction"] =
        COGNITO_CONSTANTS.MESSAGE_ACTIONS.SUPPRESS;
    }
